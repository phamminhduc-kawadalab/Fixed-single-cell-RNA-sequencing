...
req_subset <- readRDS("/Users/minhduc/Desktop/human_immune.rds")
DimPlot(req_subset, label = T, group.by = "orig.ident")
FeaturePlot(req_subset, "SEMA4D")
req_subset@meta.data

req_subset <- readRDS("/Volumes/LaCie/20251005_Data\ analysis\ /20250105_Data\ analysis\ /20250105_Bcell.rds")
...
req_subset <- subset(data.combined, idents = c("9"), invert=F)
req_subset <- subset(data.combined, idents = c("B cells"), invert=F)
req_subset
...
req_subset <- subset(req_subset, idents = c("6","7", "8"), invert =T)
...
FeaturePlot(req_subset, "Ptp4a3", label = T)
...
# Run Dorothea
req_subset <- readRDS("/Volumes/LaCie/For\ articles/20240706_B\ cell/20240607_B_cell_subset.rds")
...
req_subset1 <- subset(req_subset, idents = c("HSC_S0","HSC_S3", "HSC_S1", "HSC_S4", "HSC_S6", "HSC_S5","HSC_S7"))
...
hm <- draw(ComplexHeatmap::Heatmap(regulonActivity_byCellType_Scaled, name="Regulon activity"), row_names_gp=grid::gpar(fontsize=6)))
  ...
  saveRDS(req_subset, "/Users/lethithanhthuy/Desktop/20240607_B_cell_subset.rds")
