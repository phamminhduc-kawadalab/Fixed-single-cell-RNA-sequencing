...
req_subset <- subset(data.combined, idents = c("21"), invert=F)
...
req_subset <- subset(req_subset, idents = c("4","5","6"), invert =T)
DimPlot(req_subset,label = T, pt.size = 1.5, group.by = "seurat_clusters")  + theme(axis.text.x = element_text(size = 20, color = "black"), axis.text.y = element_text(size = 20, color = "black"), legend.text = element_text(size = 18), axis.title.y = element_text(size = 20), axis.title.x = element_text(size = 20)) +  guides(color = guide_legend(override.aes = list(size=10), ncol=1) ) + ggtitle(NULL)
...
FeaturePlot(req_subset, "Cxcr2", label = T)
VlnPlot(req_subset, "Cyp2e1", idents = c(0,12,18))
VlnPlot(req_subset, "Fcna", group.by = "group")

req_subset <- readRDS("/Volumes/LaCie/20251005_Data\ analysis\ /20250105_Data\ analysis\ /20250105_Neutrophils.rds")
DimPlot(req_subset)
...
req_subset <- readRDS("/Volumes/LaCie/For\ articles/20240706_Neutrophils\ subset/20240706_Neutrophils_subset.rds")
...
p1 <- DimPlot(req_subset, reduction = "umap", label = TRUE, pt.size = 0.5) + 
  NoLegend() + ggtitle('Cell types')
p2 <- (FeaturePlot(req_subset, features = c("Smad3")) & 
         scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle('Smad3 activity')
p2 + theme(axis.text.x = element_text(size = 20, color = "black"), axis.text.y = element_text(size = 20, color = "black"), legend.text = element_text(size = 18), axis.title.y = element_text(size = 20), axis.title.x = element_text(size = 20)) 
...
genes <- c(unlist(strsplit("Colec11",split = ", ")))
# markers required
ptsize <- 1
genes <- intersect(genes, rownames(req_subset@assays$RNA@data))
FeaturePlot(object = req_subset, features = c(genes[1]), max.cutoff = "q99", pt.size = ptsize, order = T) + scale_color_viridis_c() +  theme(axis.text.x = element_text(size = 20, color = "black"), axis.text.y = element_text(size = 20, color = "black"), legend.text = element_text(size = 18), axis.title.y = element_text(size = 20), axis.title.x = element_text(size = 20), title = element_text(size = 20)) 
...
colorer <- c("CONT"="red2","TAA"="blue", "TAARE"="forestgreen")
...
barplot = data.frame(HSC_Clusters = Idents(req_subset), Cluster = Group)
barplot$Sample <- req_subset@meta.data$Cell.types
bb=ggplot(barplot, aes(HSC_Clusters, fill=Group)) + geom_bar()  + theme_bw() + ylab("Cell count")
bb2=ggplot(barplot, aes(HSC_Clusters, fill=Group)) + geom_bar(position="fill")  + ylab("Cell composition")

bb=ggplot(barplot, aes(HSC_Clusters, fill=Group)) + geom_bar() + theme_bw() + ylab("Cell count")+ xlab(NULL)+ theme(axis.text.x = element_text(size = 20, color = "black", angle = 45, hjust = 1), axis.text.y = element_text(size = 20, color = "black"), legend.text = element_text(size = 20), axis.title.y = element_text(size = 20))
bb2=ggplot(barplot, aes(HSC_Clusters, fill=Group)) + geom_bar(position="fill")+ ylab("Cell composition")+ xlab(NULL) + theme(axis.text.x = element_text(size = 18, color = "black", angle = 45, hjust = 1), axis.text.y = element_text(size = 16, color = "black"), legend.text = element_text(size = 18), axis.title.y = element_text(size = 20))
...
plotter$Cyp2e1 <- req_subset@assays$integrated@data["Cyp2e1",]
plotter$Cyp1a2 <- req_subset@assays$integrated@data["Cyp1a2",]



plotter$group <- plotter$group
plotter$group[plotter$Cyp2e1 < 0.25] <- "TAARF"
plotter$group[plotter$Cyp1a2 < 0.25] <- "TAARF"

gg = ggplot(plotter, aes(Cyp2e1, Cyp1a2, color=group)) + 
  geom_point(size=0.5) + 
...
plotter_int <- subset(plotter,  > 0.25 & BATF > 0.25)
...
gene_umap = function(req_subset, markers, pts = 2, cols=NA){
  
  if(is.na(cols)){
    
    plot_umap =
      FeaturePlot(req_subset, features=markers, 
                  reduction = "umap", 
                  cells = sample(colnames(req_subset)), pt.size=pts, 
...
markers = c("Cyp2e1")
batf_u = gene_umap(req_subset, markers, pts=0.3)

batf_u
markers = c("Cyp1a2")
fox_u = gene_umap(req_subset, markers, pts=0.3)
...
req_subset <- SetIdent(req_subset, value = "group")
...
DoHeatmap(req_subset, features = top15$gene) + NoLegend() 
...
group_colors <- c("Control" = "red", "TAA" = "green", "RE" = "blue")
library(ComplexHeatmap)

group_bar <- HeatmapAnnotation(df = data.frame(Group = req_subset$group),
                               col = list(Group = group_colors[req_subset$group]),
                               width = unit(1, "cm"))
...
DoHeatmap(req_subset, features = top15$gene, label = F) +
  theme(axis.text.y = element_text(size = 18), legend.text = element_text(size = 18)) + scale_fill_gradientn(colors = c("green", "white", "red")) + scale_y_discrete(labels = c("Ltbp2" = "", "Apoc3"="", "Apoc1" ="", "Itgb3"="", "Saa2"="", "Clec4g"="", "Cyp2e1"="", "Gpx3"="", "Mgp"="", "Gsn"="", "Il34"="", "Fbn1"="", "Fam180a"="")) 

DoMulti

heatmap_with_bar <- Heatmap(heatmap_plot, top_annotation = group_bar)
...
levels(req_subset1) <- c("HSC_S0","HSC_S3", "HSC_S1", "HSC_S4", "HSC_S6", "HSC_S5","HSC_S7")

req_subset1$cluster,
levels = c("HEP_PC_CONT", "HEP_PC_TAA", "HEP_PC_RE")
)

levels(req_subset1) <- c("LSEC_Q1","LSEC_Q2", "LSEC_A1", "LSEC_A4", "LSEC_R1")
req_subset1@meta.data
levels(req_subset1)

DoHeatmap(req_subset1, features = top20$gene) + theme(axis.text.y = element_text(size = 8))
...
vlnPlot(res,features="extracellular.matrix.organization", split.by="Cell.types")
...
str(hsko)
  ...
  figure <- "Figure_6_and_S6/6D_motif_regulon9"
...
hm <- draw(ComplexHeatmap::Heatmap(regulonActivity_byCellType_Scaled, name="Regulon activity"), row_names_gp=grid::gpar(fontsize=6)))
  ...
  levels(req_subset1) <- c("HSC_Q0","HSC_Q3", "HSC_A1", "HSC_A4", "HSC_A6", "HSC_I5","HSC_I7")
  gene <-"Cd63"
  p <- VlnPlot(req_subset1, features = gene, split.by = "group") 
  df <- p$data
  colnames(df)[1] <- "gene"
  head(df)
  tmppos <- which(df$=="CONT")
  tmpx <- df$gene[tmppos]
  tmpy <- df$gene[-tmppos]
  ...
  "HSC_Q0","HSC_Q3", "HSC_A1", "HSC_A4", "HSC_A6", "HSC_I5","HSC_I7"
  
  # Making heatmap
  
  write.csv(regulonActivity_byCellType_Scaled, "/Users/lethithanhthuy/Desktop/HSC_regulonActivity_byCellType_Scaled.csv")
  
  heatmap <- read_csv("/Users/lethithanhthuy/Desktop/HSC_regulonActivity_byCellType_Scaled.csv")
  
  selected_cell_types <- c("HSC_Q0","HSC_Q3", "HSC_A1", "HSC_A4", "HSC_A6", "HSC_I5","HSC_I7")
  ...
  DefaultAssay(data.combined)<- "RNA"
  features <- c("Cygb", "Lrat", "Des", "Col1a1", "Col1a2", "Eln", "Alb", "Krt7", "Clec4f", "C1qb", "Stab2", "Bmp2")
  ...
  EnhancedVolcano(c9,
                  ...
                  x = 'avg_log2FC',
                  y = 'p_val',
                  selectLab = c('Col1a1','Col1a2', "Ccdc80", 'Col3a1', "Ltbp2", 'Mup17', "Cyp2e1", "Fabp1"),
                  xlab = bquote(~Log[2]~ 'fold change'),
                  pCutoff = 10e-14,
                  FCcutoff = 1.0,
                  pointSize = 4.0,
                  labSize = 6.0,
                  subtitle = element_blank(), title = "HSCs_RE (left) vs. HSCs_TAA (right)",
                  caption = element_blank(),
                  labCol = 'black',
                  labFace = 'bold',
                  boxedLabels = TRUE,
  ...
  data.combined <- readRDS("/Users/lethithanhthuy/Desktop/DUC/data_combined.rds")
  saveRDS(req_subset, "/Users/lethithanhthuy/Desktop/20240706_Neutrophils_subset.rds")
