...
req_subset <- subset(data.combined, idents = c("26", "12"), invert=F)
req_subset <- subset(data.combined, idents = c("NKT cell"), invert=F)
req_subset
...
# Convert the cluster IDs to a factor and reassign levels
req_subset$new.cluster.ids <- factor(req_subset$new.cluster.ids, levels = new.cluster.ids)
levels(req_subset$new.cluster.id) <- new.cluster.ids


class(req_subset)
req_subset$ident
head(metadata(req_subset))
head(MetaData(req_subset))
req_subset[["seurat_clusters"]] <- Idents(object = req_subset)
...
cluster16.markers <- FindMarkers(req_subset, ident.1 = 16, min.pct = 0.25)
head(cluster16.markers, n = 30)
cluster17.markers <- FindMarkers(req_subset, ident.1 = 17, min.pct = 0.25)
head(cluster17.markers, n = 30)
cluster18.markers <- FindMarkers(req_subset, ident.1 = 18, min.pct = 0.25)
head(cluster18.markers, n = 30)
cluster19.markers <- FindMarkers(req_subset, ident.1 = 19, min.pct = 0.25)
head(cluster19.markers, n = 30)
...
write.csv(enriched, "/Users/lethithanhthuy/Desktop/Enrich_Cluster13.csv")
write.csv(enriched, "/Users/lethithanhthuy/Desktop/Enrich_Cluster14.csv")
...
write.csv(enriched, "/Users/lethithanhthuy/Desktop/Enrich_Cluster17.csv")
write.csv(enriched, "/Users/lethithanhthuy/Desktop/Enrich_Cluster18.csv")
write.csv(enriched, "/Users/lethithanhthuy/Desktop/Enrich_Cluster19.csv")
...
bb=ggplot(barplot, aes(HEP_Clusters, fill=Group)) + geom_bar()  + theme_bw() + ylab("Cell count")
bb2=ggplot(barplot, aes(HEP_Clusters, fill=Group)) + geom_bar(position="fill")  + ylab("Cell composition")

bb + theme(axis.text.x = element_text(size = 12, color = "black", angle = 45, hjust = 1), axis.text.y = element_text(size = 12, color = "black"), legend.text = element_text(size = 12))
bb2 + theme(axis.text.x = element_text(size = 16, color = "black", angle = 45, hjust = 1), axis.text.y = element_text(size = 16, color = "black"), axis.title.x = element_blank(), axis.title.y = element_text(size = 16, color = "black"), legend.text = element_text(size = 16))
...
meta <-req_subset@meta.data

# Run heatmap for subcluster
